...
double prev_posn[3] = {0.0, 0.0, 0.0};
void loop() {
  double A = 0.25, V = 0.15, W = 1.85;
  ...
  while(dist<A){
    ...
    delay(10);
    mybot.update_odom(10);
    dist = sqrt(pow((mybot._posn[0] - prev_posn[0]), 2) + pow((mybot._posn[1] - prev_posn[1]), 2));
  }
  mybot.move(0, 0);
  delay(100);
  prev_posn[0] = mybot._posn[0];
  prev_posn[1] = mybot._posn[1];
  prev_posn[2] = mybot._posn[2];
  while(angle<1.57){
    //for a square
    mybot.move(0, W);
    delay(10);
    mybot.update_odom(10);
    angle += mybot._posn[2] - prev_posn[2];
    prev_posn[0] = mybot._posn[0];
    prev_posn[1] = mybot._posn[1];
    prev_posn[2] = mybot._posn[2];
  }
